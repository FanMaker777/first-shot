...
class PlayScene:
    ...
    def update(self):
        ...
        for enemy in self.game.enemies.copy():
            ...
            # 自機と敵の当たり判定を行う
            if self.game.player is not None and check_collision(self.game.player, enemy):
                self.game.player.add_damage()  # 自機にダメージを与える

        # 自機の弾を更新する
        for bullet in self.game.player_bullets.copy():
            bullet.update()
            ...
            for enemy in self.game.enemies.copy():
                if check_collision(enemy, bullet):
                    ...
                    if self.game.player is not None:  # 自機が存在する時
                        self.game.player.sound_timer = 5  # 弾発射音を止める時間を設定する
        ...
        for bullet in self.game.enemy_bullets.copy():
            ...
            # プレイヤーと敵の弾の当たり判定を行う
            if self.game.player is not None and check_collision(self.game.player, bullet):
